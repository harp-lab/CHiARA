...
int MPICH_Allreduce_inter_reduce_exchange_bcast(const char* sendbuf, char* recvbuf, int count,
                                                MPI_Datatype datatype, MPI_Op op, MPI_Comm comm);
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        if (overwrite || !exists) {
            ...
            csv << "algorithm_name,k,nprocs,count,time,is_correct\n";
        } else {
    ...
    for (int i = 0; i < n_iter; ++i) {
        ...
        run_no_k("MPICH_Allreduce_inter_reduce_exchange_bcast",
                 count, MPICH_Allreduce_inter_reduce_exchange_bcast,
                 MPI_COMM_WORLD, csv, rank, nprocs);
